...
\t<td style="border:0"><div class="epp-primary bold">{{ doc.company or "" }}</div><div>{{ title }}</div></td>
	...
	"terms/notes.html": """{% if doc.remarks or doc.notes %}<div class="epp-box">{{ doc.remarks or doc.notes }}</div>{% endif %}""",
...
def pick_components(profile, theme: str) -> list[str]:
	...
	elif profile.doc_type in ("Payment Entry", "Journal Entry"):
		parts += ["header_document_number", "party_contact" if profile.party_field else "notes_block", "totals_basic"]
